...
class ContainerValidator:
    ...
    TASK_CONFIGS = {
        ...
        "task3": TaskConfig(
            sequences=["t1", "t2"], min_sequences=2, output_type=OutputType.REGRESSION
        ),
    }

    def __init__(
        self, container_path: Path, task: str, apptainer_cmd: str = "apptainer"
    ):
    ...
    def _check_gpu_support(self) -> Tuple[bool, str]:
        ...
        success, msg = self._run_command(
            [self.apptainer_cmd, "exec", "--nv", str(self.container_path), "nvidia-smi"]
        )
        if success:
    ...
    def _find_session_directory(self, subject_dir: Path) -> Path:
        ...
        if not scan_dir.exists():
            raise ValidationError(
                f"Expected {self.SESSION_DIR} directory not found in {subject_dir}"
            )
    ...
    def _build_prediction_command(
        ...
        # Add output argument
        output_ext = (
            ".nii.gz" if self.config.output_type == OutputType.SEGMENTATION else ".txt"
        )
    ...
    def predict_subject(
        self, subject_id: str, subject_dir: Path, output_dir: Path
    ) -> Tuple[bool, str]:
        ...
        try:
            ...
            cmd = self._build_prediction_command(
                subject_dir, sequences, output_dir, subject_id
            )
    ...
    def predict_all(self, data_dir: Path, output_dir: Path) -> Tuple[bool, str]:
        ...
        try:
            ...
            print(
                f"[INFO] Found {len(subject_dirs)} subjects: {[d.name for d in subject_dirs]}"
            )
    ...
    def _validate_text_outputs(self, output_dir: Path) -> Tuple[bool, str]:
        ...
        return (
            True,
            f"All {len(txt_files)} {self.config.output_type.value} outputs valid",
        )

    def _validate_segmentation_outputs(
        self, output_dir: Path, data_dir: Path
    ) -> Tuple[bool, str]:
    ...
    def validate_outputs(
        self, output_dir: Path, data_dir: Optional[Path] = None
    ) -> Tuple[bool, str]:
...
Examples:
  ...
  python main.py --task task1 --container /path/to/task1container.sif --data-dir /path/to/data --output-dir /path/to/output

  # Only validate environment
  python main.py --task task3 --container /path/to/task3container.sif --validate-env-only
  ...
  python main.py --task task1 --output-dir /path/to/output --data-dir /path/to/data --validate-outputs-only
    ...
    parser.add_argument(
        "--container", type=Path, help="Path to the container file (.sif)"
    )
    ...
    parser.add_argument(
        "--output-dir", type=Path, help="Path to the output directory for predictions"
    )
...
def run_environment_validation(
    validator: ContainerValidator, skip_gpu_check: bool = False
) -> bool:
...
def run_output_validation(
    validator: ContainerValidator, output_dir: Path, data_dir: Optional[Path] = None
) -> bool:
        ...
        if args.validate_env_only:
            ...
            if success:  # Only proceed if environment validation passed
                ...
                if success:  # Only validate outputs if predictions succeeded
                    # Output validation
                    success = run_output_validation(
                        validator, args.output_dir, args.data_dir
                    )
                    overall_success = overall_success and success
